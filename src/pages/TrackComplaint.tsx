      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
        <ScrollReveal direction="up" delay={100}>
          <div className="mb-10 animate-fade-up">
            <h1 className="text-4xl font-bold text-gray-900 mb-3 tracking-tight">{T.track_title}</h1>
        ...
        <ScrollReveal direction="up" delay={200}>
          <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6 shadow-sm hover:shadow-md transition-shadow duration-300">
            ...
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-transparent font-mono input-premium"
              />
              <button
                onClick={() => handleSearch()}
                disabled={loading || !query.trim()}
                className="flex items-center gap-2 px-6 py-3 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl btn-premium active:scale-95 transition-all group"
              >
                {loading ? <Loader size={16} className="animate-spin" /> : <Search size={16} className="transition-transform duration-200 group-hover:scale-110" />}
                {T.track_search_btn}
              </button>
            </div>
          </div>
        ...
        {notFound && (
          <ScrollReveal direction="scale">
            <div className="bg-white rounded-2xl border border-red-100 p-10 text-center shadow-sm hover:shadow-md transition-shadow duration-300">
              <AlertTriangle size={40} className="text-red-400 mx-auto mb-3 animate-bounce" />
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 text-white text-sm font-semibold rounded-xl hover:bg-orange-600 shadow-md btn-premium active:scale-95 transition-all group"
              >
                {T.track_file_new} <ArrowRight size={14} className="transition-transform duration-200 group-hover:translate-x-0.5" />
              </button>
            </div>
          </ScrollReveal>
        )}
        ...
        {complaint && (
          <div className="space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {complaint.similar_count > 1 && (
                        <span className="flex items-center gap-1 text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-semibold">
                          <GitBranch size={10} className="animate-pulse" /> {complaint.similar_count} {T.track_in_cluster}
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-4 pt-5 border-t border-gray-50">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900 flex items-center gap-1"><MapPin size={11} className="text-orange-500" />{complaint.area}</p>
                  </div>
                  <div>
            ...
            {complaint.status === 'ESCALATED' && (
              <ScrollReveal direction="scale">
                <div className="bg-white rounded-2xl border border-red-200 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                    ...
                    {STATUS_STEPS.map((step, i) => {
                      ...
                      return (
                        <div key={step} className="relative flex items-start gap-4 pb-6 last:pb-0">
                          ...
                          <div className={`pt-1.5 transition-opacity duration-300 ${isCompleted ? 'opacity-100' : 'opacity-45'}`}>
            ...
            {complaint.similar_count > 1 && (
              <ScrollReveal direction="scale">
                <div className="bg-white rounded-2xl border border-indigo-100 p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-10 h-10 rounded-xl bg-indigo-100 flex items-center justify-center">
                      <Users size={18} className="text-indigo-600 animate-pulse" />
                    </div>
                    <div>
                  ...
                  <div className="bg-indigo-50 rounded-xl p-4 text-center">
                    <span className="text-5xl font-black text-indigo-600"><CountUp end={complaint.similar_count} /></span>
                  ...
                  <ScrollReveal key={c.id} direction="up" delay={i * 80}>
                    <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            className="font-mono text-sm font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded hover:bg-orange-100 transition-colors duration-200"
                          >
                            {c.complaint_number}
                          </button>
                      ...
                      <p className="text-xs text-gray-400 mt-3 flex items-center gap-1.5">
                        <Clock size={12} className="group-hover:animate-pulse" />
